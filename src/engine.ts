...
export class Solver {
  ...
  constructor(
    ...
    this.verlets = [...Array(nVerlets)].map(() => {
      const r = randInt(2, 2);
      const pos = {
        x: randInt(r * 2, canvasW - r * 2),
        y: randInt(r * 2, canvasH - r * 2),
      };
      return new Verlet(
        pos,
        pos,
    ...
    this.behaviourMatrix = behaviourMatrix.map((arr) => {
      return arr.map((val) => val * 0.00001);
    });

    const maxRoe = this.verlets.reduce(
      (acc, curr) => (curr.roe > acc ? curr.roe : acc),
      0,
    );
  ...
  solveCollisions(cells: Array<Array<number>>) {
    for (let i = 1; i < this.spaceParition.nH - 1; i += 2) {
      for (let j = 1; j < this.spaceParition.nV - 1; j += 2) {
        ...
        const verletIndices = [
          ...cells[cellIdx - 1 - this.spaceParition.nH],
          ...cells[cellIdx - this.spaceParition.nH],
          ...cells[cellIdx + 1 - this.spaceParition.nH],
          ...cells[cellIdx - 1],
          ...cells[cellIdx],
          ...cells[cellIdx + 1],
        ...
        for (let k = 0; k < verletIndices.length; k++) {
          ...
          for (let l = 0; l < verletIndices.length; l++) {
            ...
            // -- Effect
            const v1Tov2Factor =
              this.behaviourMatrix[this.colorIndices[verlet.color.string]][
                this.colorIndices[_verlet.color.string]
              ];
            const v2Tov1Factor =
              this.behaviourMatrix[this.colorIndices[_verlet.color.string]][
...
export class Renderer {
  ...
  constructor(gl: WebGLRenderingContext) {
    ...
    this.attributes['a_position'] = gl.getAttribLocation(
      this.program,
      'a_position',
    );
  ...
  drawVerlet(verlet: Verlet) {
    ...
    for (let i = 1; i <= steps; i++) {
      ...
      let newX =
        verlet.r * Math.cos(degToRad((360 / steps) * i)) + verlet.posCurr.x;
      let newY =
        verlet.r * Math.sin(degToRad((360 / steps) * i)) + verlet.posCurr.y;

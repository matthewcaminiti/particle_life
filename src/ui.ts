...
export const initStatWindow = (): statWindow => {
  ...
  const collisionEle = document.getElementById(
    'collision-ele',
  ) as HTMLDivElement;
...
export const initControlPanel = (numVerlets: number): controlPanel => {
  ...
  const behaviourMatrix: Array<Array<number>> = [
    ...Array(Object.keys(colors).length),
  ].map(() => [...Array(Object.keys(colors).length)].map(() => 0));
  const colorIndices: Record<string, number> = Object.entries(colors).reduce(
    (acc, [_, v4]) => {
  ...
  Object.values(colors).forEach((color) => {
    const turnedOn = Math.random() >= 0.3;
    if (turnedOn) chosenColors.push(color.string);

    const row = document.createElement('tr');

    const col = document.createElement('td');
    col.setAttribute('class', 'color-matrix-cell color-checkbox-cell');
    col.setAttribute(
      'style',
      `background-color: ${COLOR_NAMES[color.string]};`,
    );
    ...
    Object.values(colors).forEach((_color) => {
      ...
      if (turnedOn) {
        ...
        behaviourMatrix[colorIndices[color.string]][
          colorIndices[_color.string]
        ] = val;
        col.textContent = val !== 0 ? val.toString() : '';
      }
      col.setAttribute(
        'id',
        `behaviour-${COLOR_NAMES[color.string]}-${COLOR_NAMES[_color.string]}`,
      );
      row.appendChild(col);
    });

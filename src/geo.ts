...
export class circle {
  ...
  doesAffect(dt: number, other: circle) {
    ...
    const dist = Math.sqrt(
      (nextOtherY - nextThisY) ** 2 + (nextOtherX - nextThisX) ** 2,
    );
  ...
  reactTo(other: circle, factor: number) {
    ...
    this.velocity = {
      x: normD.x * proportion * factor,
      y: normD.y * proportion * factor,
    };
  ...
  collide(other: circle) {
    ...
    this.velocity = {
      x: damping * other.speed * normD.x * -1,
      y: damping * other.speed * normD.y * -1,
    };
    other.velocity = {
      x: damping * thisSpeed * normD.x,
      y: damping * thisSpeed * normD.y,
    };
...
export class Verlet {
  ...
  updatePosition(dt: number) {
    const v = v2.scale(v2.sub(this.posCurr, this.posOld), 0.98);

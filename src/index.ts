...
const main = () => {
    ...
    checkbox.addEventListener('click', () => {
      ...
      document.querySelectorAll("td[id^='behaviour-']")?.forEach((cell) => {
        ...
        if (c1 === color || c2 === color) {
          cell.textContent = '';
          if (c1 === color)
            control.behaviourMatrix[control.colorIndices[colors[c1].string]][
              control.colorIndices[colors[c2].string]
            ] = 0;
          if (c2 === color)
            control.behaviourMatrix[control.colorIndices[colors[c2].string]][
              control.colorIndices[colors[c1].string]
            ] = 0;
        }
      });

      const filtered = control.chosenColors.filter(
        (c) => c !== colors[color].string,
      );
      if (filtered.length !== control.chosenColors.length)
        control.chosenColors = filtered;
      else control.chosenColors.push(colors[color].string);
    });
  });
  ...
  document.querySelectorAll("td[id^='behaviour-']")?.forEach((cell) => {
    ...
    cell.addEventListener('click', () => {
      ...
      const c1Checkbox = document.getElementById(
        `checkbox-${c1}`,
      ) as HTMLInputElement;
      ...
      const c2Checkbox = document.getElementById(
        `checkbox-${c2}`,
      ) as HTMLInputElement;
      ...
      cell.textContent =
        control.behaviourMatrix[c1Idx][c2Idx] !== 0
          ? control.behaviourMatrix[c1Idx][c2Idx].toString()
          : '';
    });
  });
